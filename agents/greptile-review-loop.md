...
2. **SCORE >= 4 AND NEW_ISSUES == 0 BUT get_merge_request shows old unaddressed comments** →
   ...
   - If no (some old comments were never handled) → **First, set `NEW_ISSUES = [old unaddressed comments not in HANDLED_IDS]` (this explicitly updates the variable before proceeding)** → go to **STEP 4: FIX STEP**
