...
**Use `gh` CLI for non-Greptile operations:**
- Git operations (commit, push, branch)
- PR creation and merging (`gh pr create`, `gh pr merge`)
- CI check monitoring (`gh run list`, `gh run view`)
...
1. **Wait 30 seconds** for Greptile to detect the push and start a review.

2. **Poll using MCP every 30 seconds for up to 2 minutes:**
...
**IMPORTANT: Use SEPARATE Bash calls for sleep and MCP calls for checking.** Do NOT combine
them into a single long-running command.

**Poll every 30 seconds, max wait 15 minutes (30 iterations):**

Each iteration, call `mcp__plugin_greptile_greptile__list_code_reviews` with:
- `name` = REPO, `remote` = "github", `defaultBranch` = DEFAULT_BRANCH, `prNumber` = PR_NUMBER

Check the latest review's status:

1. **COMPLETED** → Review is done → go to **STEP 3: ANALYZE PR STATE**
2. **FAILED** → Review failed → go to **TRIGGER REVIEW** (retry)
3. **PENDING / REVIEWING_FILES / GENERATING_SUMMARY** → Still in progress.
   Run `sleep 30` in Bash, then poll again.
